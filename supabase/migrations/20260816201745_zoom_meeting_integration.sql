-- Per-staff Zoom OAuth for meeting join links (independent of calendar vendor).
-- Tokens stay in private.zoom_secrets (service_role RPCs only).
-- Join URL is stored on booking_appointments.meet_join_url; meeting id on conference_id.

alter table public.staff_booking_integrations
  drop constraint if exists staff_booking_integrations_meeting_provider_check;

alter table public.staff_booking_integrations
  add constraint staff_booking_integrations_meeting_provider_check
  check (
    meeting_provider is null
    or meeting_provider in ('google_meet', 'teams', 'zoom')
  );
...
comment on table public.zoom_connections is
  'One Zoom connection per staff user for booking meeting links. OAuth tokens live in private.zoom_secrets.';
...
grant select, insert, update, delete on private.zoom_secrets to service_role;
revoke all on private.zoom_secrets from public, anon, authenticated;
