...
export type MicrosoftCalendarSecretsPublic = {
...
export async function getMicrosoftCalendarSecrets(connectionId: string) {
  const { data, error } = await admin().rpc("get_microsoft_calendar_secrets", {
    p_connection_id: connectionId,
  });
  if (error) {
    console.error("get microsoft secrets:", error.message);
    return null;
  }
  ...
  } satisfies MicrosoftCalendarSecretsPublic;
}

export async function upsertMicrosoftCalendarSecrets(input: {
  connectionId: string;
  ...
  const { error } = await admin().rpc("upsert_microsoft_calendar_secrets", {
  ...
  if (error) {
    console.error("upsert microsoft secrets:", error.message);
    throw new Error("microsoft_secrets_save_failed");
  }
}

export async function updateMicrosoftCalendarSecrets(
  connectionId: string,
  patch: {
    accessTokenEncrypted?: string | null;
    accessTokenExpiresAt?: Date | null;
  ...
  const { error } = await admin().rpc("patch_microsoft_calendar_secrets", {
    p_connection_id: connectionId,
  ...
  if (error) {
    console.error("patch microsoft secrets:", error.message);
    throw new Error("microsoft_secrets_save_failed");
  }
}
