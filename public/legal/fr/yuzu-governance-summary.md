# Gouvernance des renseignements personnels — résumé public

**%OPERATOR_AS%** — exploitant de %PRODUCT_NAME%  
**Responsable de la protection des renseignements personnels :** Adrien Yvin — %PRIVACY_EMAIL%  
**Dernière mise à jour :** 5 septembre 2026

Ceci est la description claire des politiques de gouvernance exigée par la *Loi sur la protection des renseignements personnels dans le secteur privé* du Québec (Loi 25). Les règles opposables aux utilisateurs figurent dans la [politique de confidentialité](/fr/privacy).

## Qui fait quoi

- Votre **cabinet** décide des fins de collecte et est responsable du dossier d’immigration (consentement, mandats, devoirs du Collège, avis aux clients).
- **%OPERATOR_NAME%** exploite **%PRODUCT_NAME%**. Nous traitons les renseignements clients selon les instructions du cabinet. Nous sommes responsables de la protection de la plateforme et des renseignements de compte du personnel nécessaires à l’exploitation.

## Ce que nous protégeons

Les dossiers peuvent comprendre des noms, coordonnées, passeports, questionnaires, documents, notes, rendez-vous, paiements et des renseignements sur la famille (y compris des mineurs). Nous les traitons comme sensibles.

## Règles principales

1. Collecte et utilisation limitées à l’exploitation de %PRODUCT_NAME% et aux fins de la politique de confidentialité. Pas de vente de renseignements personnels.  
2. Accès limité dans chaque cabinet ; les rôles Admin et Gestionnaire de cas distinguent l’administration de l’espace et le travail des dossiers.  
3. Chiffrement en transit. Base de données, authentification et fichiers au Canada (région de Montréal). Chiffrement applicatif de nombreux champs et documents, clé par cabinet.  
4. Outils facultatifs (Google, Microsoft, Zoom, Square, Stripe Connect, Sage Comptabilité, analytique) **désactivés** tant que personne n’y consent. La facturation Stripe de l’abonnement du cabinet ne s’applique que lorsque le cabinet paie le logiciel.  
5. Conservation des dossiers fermés six ans par défaut (Collège), puis destruction sécurisée.  
6. Aide aux cabinets pour l’accès, la rectification, la portabilité et la suppression. Vous pouvez aussi écrire à %PRIVACY_EMAIL%. Objectif : 30 jours.  
7. Registre des incidents de confidentialité. Avis à la CAI, au CPVP, aux cabinets et aux personnes lorsque la loi l’exige.  
8. Évaluation des facteurs relatifs à la vie privée avant un changement majeur de système et avant une communication hors Québec.

## Pour aller plus loin

- Politique de confidentialité : `/fr/privacy`  
- Sous-traitants : [yuzu-subprocessors.pdf](./yuzu-subprocessors.pdf)  
- Modèles pour cabinets : listés sur la page de confidentialité et sous Paramètres → Sécurité dans %PRODUCT_NAME%
